    ...
    def _train_meta_learner(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for name, model, timeout in models:
            ...
            if pred is not None:
                ...
                # Timed-out/failed models are skipped entirely rather than
                # backfilled with a constant column - a constant feature
                # contributes no signal but still lets the meta-learner fit
                # a (spurious) coefficient on it and advertises a model that
                # didn't actually participate.
                print(f"  {name} unavailable for meta-learner training - skipping", flush=True)
