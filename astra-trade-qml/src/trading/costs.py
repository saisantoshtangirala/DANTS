...
class CostCalculator:
    ...
    def round_trip_cost_pct(self, notional: float, side: str = "BUY", delivery: bool = False) -> float:
        """Round-trip cost as a fraction of notional, for a representative trade size.

        Used by vectorized backtests that only have price returns (no discrete
        quantity/price legs) but still need a realistic cost drag per trade."""
        if notional <= 0:
            return 0.0
        return self.round_trip_cost(notional, notional, quantity=1, side=side, delivery=delivery) / notional
